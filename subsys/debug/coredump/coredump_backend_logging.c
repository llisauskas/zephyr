...
static int coredump_logging_backend_cmd(enum coredump_cmd_id cmd_id, void *arg)
{
	int ret;
...
static int cmd_coredump_error_get(const struct shell *shell, size_t argc,
				  char **argv)
{
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_coredump_error_clear(const struct shell *shell, size_t argc,
				    char **argv)
{
	error = 0;
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_coredump_error,
			       SHELL_CMD(get, NULL, "Get Coredump error",
